...
//      gst_parse_launch ("videotestsrc ! edgetv ! videoconvert ! autovideosink", &error);
        // small screen with fuzzy
//      gst_parse_launch ("videotestsrc ! radioactv ! videoconvert ! autovideosink", &error);
        // horizontal lines
//        gst_parse_launch ("videotestsrc ! revtv ! videoconvert ! autovideosink", &error);
        // wavely image
//        gst_parse_launch ("videotestsrc ! rippletv ! videoconvert ! autovideosink", &error);
        // windmill
//        gst_parse_launch ("videotestsrc ! shagadelictv ! videoconvert ! autovideosink", &error);
        // dim to bright
//        gst_parse_launch ("videotestsrc ! streaktv ! videoconvert ! autovideosink", &error);
        // twilight zone
//        gst_parse_launch ("videotestsrc ! vertigotv ! videoconvert ! autovideosink", &error);
        // time overlay
//        gst_parse_launch ("videotestsrc ! vertigotv ! videoconvert "
...
gst_native_class_init (JNIEnv * env, jclass klass)
{
  ...
  if (!custom_data_field_id || !set_message_method_id
     ...
     */
    LOGE("%s", "tutorial-3: The calling class does not implement "
...
JNI_OnLoad (JavaVM * vm, void *reserved)
{
  ...
  if ((*vm)->GetEnv (vm, (void **) &env, JNI_VERSION_1_4) != JNI_OK) {
    LOGE("%s", "tutorial-3: Could not retrieve JNIEnv");
    return 0;
  }
