...
state_changed_cb (GstBus * bus, GstMessage * msg, CustomData * data)
{
  ...
  if (GST_MESSAGE_SRC (msg) == GST_OBJECT (data->pipeline)) {
    gchar *message = g_strdup_printf ("现况：%s",
...
app_function (void *userdata)
{
  ...
  data->pipeline = gst_parse_launch
      ("audiotestsrc wave=ticks "
       "apply-tick-ramp=true tick-interval=100000000 "
       "freq=10000 volume=0.4 marker-tick-period=10 "
       "sine-periods-per-tick=20 ! autoaudiosink", &error);
//      ("audiotestsrc ! audioconvert ! audioresample ! autoaudiosink", &error);
//      ("audiotestsrc freq=1000 num-buffers=100 ! autoaudiosink", &error);
  if (error) {
    gchar *message =
